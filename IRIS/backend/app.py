...
# Added turn off feature and updated wake phrases for iris and how are u also
#added gemini api , fixed weather issue
from flask import Flask, request, jsonify
from flask_cors import CORS
from datetime import datetime
import requests
...
def extract_city_from_weather_command(message):
    ...
    for pattern in patterns:
        ...
        if match:
            ...
            city = re.sub(r'\b(the|is|today|tomorrow|now|current)\b', '', city).strip()
...
def voice():
    ...
    if 'weather' in message:
        try:
            # First try to extract city from the voice command
            requested_city = extract_city_from_weather_command(message)
            
            if requested_city:
                location = requested_city
                print(f"Using city from voice command: {location}")
            else:
                # Fallback to client's location if no city specified
                location = get_client_location()
                print(f"Using client's location: {location}")
